...
class _PatientDetailScreenState extends State<PatientDetailScreen> {
  ...
  Future<void> _addPrescription() async {
    final medName = _prescriptionController.text.trim();
    ...
    try {
      ...
      await _apiService.createPrescription({
        "patientId": widget.patient.id,
        "doctorId": doctor.id, // Add the doctor's ID to the payload
        "medicines": [
          {"name": medName, "status": "prescribed"}
        ],
      });
  ...
  Future<void> _updateMedicineStatus(
      String prescriptionId, String medicineId, String newStatus) async {
    try {
      await _apiService.updateMedicineStatus(
          prescriptionId, medicineId, {"status": newStatus});
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        child: Text(
          ...
          style: TextStyle(
              color: message.isFromUser
                  ...
                  : Theme.of(context).colorScheme.onSurface), // AI message text
        ),
      ),
    );
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton.icon(
            ...
            label: Text(langProvider.t('logout'),
              ...
              child: Column(
                children: [
                  if (prescriptions.isEmpty)
                    Text(langProvider.t('noPrescribed'),
                          ...
                          child: ListTile(
                            ...
                            trailing: PopupMenuButton<String>(
                              onSelected: (String newStatus) {
                                _updateMedicineStatus(medInfo.prescriptionId,
                                    medInfo.medicine.id, newStatus);
                              },
                              itemBuilder: (BuildContext context) =>
                                  <PopupMenuEntry<String>>[
                                PopupMenuItem<String>(
                                  value: 'current',
                                  child: Text(langProvider.t('markAsCurrent')),
                                ),
                                PopupMenuItem<String>(
                                  value: 'past',
                                ...
                                PopupMenuItem<String>(
                                  value: 'prescribed',
                                  child:
                                      Text(langProvider.t('markAsPrescribed')),
                                ),
                              ],
                            ),
                          ),
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                              hintText: langProvider.t('newMedicineHint')),
                        ),
                      ),
                      IconButton(
                          icon: const Icon(Icons.add_circle_outline),
                          onPressed: _addPrescription,
                          tooltip: langProvider.t('addPrescriptionTooltip'))
                    ],
                  ),
                ],
              ),
            ...
            SectionCard(
              ...
              child: Column(
                children: history.isEmpty
                    ? [
                        Text(langProvider.t('noHistoryDetails'),
                            style: const TextStyle(color: AppTheme.textMuted))
                      ]
                    : history
                        .map((h) => Card(
                              child: ListTile(
                                leading: const Icon(Icons.history_edu_outlined,
                                    color: AppTheme.primary),
                                title: Text(h.illnessName.isNotEmpty
                                    ? h.illnessName
            ...
            SectionCard(
              ...
              child: Column(
                children: dailyReadings.isEmpty
                    ? [
                        Text(langProvider.t('noReadingsForChart'),
                            style: const TextStyle(color: AppTheme.textMuted))
                      ]
                    : dailyReadings
                        .map((r) => Card(
                  ...
                  ...notes.map((n) => Card(
                        child: ListTile(
                          leading: const Icon(Icons.note_alt_outlined,
                              color: AppTheme.primary),
                          title: Text(n.noteText.isNotEmpty
                              ? n.noteText
